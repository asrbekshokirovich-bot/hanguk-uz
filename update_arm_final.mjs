...
async function run() {
  console.log("Uploading Final ARM APK to Supabase...");
  const pk = fs.readFileSync('C:\\Users\\user\\Desktop\\hanguk_v1.0.1_arm.apk');
  
  const { error } = await supabase.storage.from('updates').upload('hanguk_v1.0.1_arm.apk', pk, {
  ...
  const publicUrl = supabase.storage.from('updates').getPublicUrl('hanguk_v1.0.1_arm.apk').data.publicUrl;
