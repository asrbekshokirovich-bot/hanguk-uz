...
import {
  FileText, Trash2, Download, Loader2, Sparkles, RefreshCw,
...
export function StudentAITranslationPage() {
  const { user } = useAuth();
  const { documentTypes, loading: loadingTypes } = useTranslationTraining();
  const { translating, runTranslation, createTranslationJob, saveTranslationResult, updateJobStatus, deleteJob } = useDocumentTranslation();

  const [myDocs, setMyDocs] = useState<DocumentRow[]>([]);
  ...
  const fetchMyDocs = useCallback(async () => {
    if (!user) return;
    const { data } = await supabase.from('documents').select('*').eq('student_id', user.id).order('created_at', { ascending: false });
    setMyDocs(data ?? []);
  }, [user]);
  ...
  useEffect(() => { fetchMyDocs(); fetchJobs(); }, [fetchMyDocs, fetchJobs]);

  useEffect(() => {
    if (!selectedType || myDocs.length === 0) { setSourceDocId(''); setAutoSupporting([]); return; }
    const resolved = resolveTranslationInputs(selectedType.code, myDocs, 'uz');
    ...
    setExtraFiles([]);
  }, [selectedTypeId, myDocs]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const uploadFile = async (file: File, kind: string): Promise<string> => {
    const path = `jobs/${user!.id}/${kind}_${Date.now()}_${file.name.replace(/[^\w.-]/g, '_')}`;
  ...
  const handleTranslate = async () => {
    if (!user || !selectedTypeId) { toast.error('Hujjat turini tanlang'); return; }
    ...
    try {
      ...
      if (uploadedSource) {
        ...
        const doc = myDocs.find((d) => d.id === sourceDocId)!;
      ...
      const job = await createTranslationJob(user.id, selectedTypeId, source.path, sourceDocumentId, false);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold">AI Tarjima</h2>
          <p className="text-sm text-muted-foreground">Hujjatlaringizni inglizchaga tarjima qiling</p>
        </div>
      </div>
              ...
              <div className="space-y-2">
                ...
                {uploadedSource ? (
                  ...
                  <Select value={sourceDocId} onValueChange={setSourceDocId}>
                    <SelectTrigger>
                      <SelectValue placeholder="Hujjatlarimdan tanlang..." />
                    </SelectTrigger>
                    <SelectContent>
                      {myDocs.length === 0
                        ? <div className="px-2 py-2 text-sm text-muted-foreground">Hujjat topilmadi</div>
                        : myDocs.map((d) => <SelectItem key={d.id} value={d.id}>{docLabel(d)}</SelectItem>)
                      }
                    </SelectContent>
                  </Select>
                )}
          ...
          <CardContent className="pt-4 pb-4">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div className="flex items-center gap-2 text-green-700 dark:text-green-400">
                ...
                <div>
                  <p className="font-medium">Tarjima tayyor!</p>
                  <p className="text-xs text-muted-foreground">Word fayli (.docx) yuklab olishga tayyor</p>
                </div>
              </div>
              <Button
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-medium text-muted-foreground">Tarjimalar tarixi</h3>
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-medium text-sm">{job.document_type?.name_uz ?? "Noma'lum"}</span>
