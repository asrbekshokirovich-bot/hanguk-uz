-- Run the lead SLA watchdog every minute.
--
-- The threshold it enforces is 10 minutes, not a day, so it needs a much
-- tighter cadence than the hourly channel/infra health jobs — every minute
-- keeps the alert's own latency negligible next to the SLA it is watching,
-- and matches the second-precision countdown now shown in the CRM.
--
...
begin
  ...
  perform cron.unschedule('lead-sla-check-1min')
  where exists (select 1 from cron.job where jobname = 'lead-sla-check-1min');
