...
async function run() {
  await supabase.storage.updateBucket('updates', { public: true, fileSizeLimit: 100000000 });
  console.log("Bucket updated to 100MB!");
  
  console.log("Uploading Universal APK...");
  const pk = fs.readFileSync('C:\\Users\\user\\Desktop\\Anti gravity\\hanguk\\hanguk_app\\build\\app\\outputs\\flutter-apk\\app-release.apk');
  
  const { error } = await supabase.storage.from('updates').upload('hanguk_v1.0.1.apk', pk, {
  ...
  const publicUrl = supabase.storage.from('updates').getPublicUrl('hanguk_v1.0.1.apk').data.publicUrl;
